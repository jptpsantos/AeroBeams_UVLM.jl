    ...
    for isurf = 1:nsurf
        ...
        for I in cr
            ...
            for jsurf = 1:nsurf
                ...
                # Skip cross-surface wake convection only for non-interaction runs.
                if !interaction && (interaction_id[isurf] != interaction_id[jsurf])
                    jΓ += Ns # Important: Skip calculation but advance Γ index
                    continue
                end
...
#"""
#    get_wake_velocities!(wake_velocities, surfaces, wakes, ref, fs, Γ, ...; interaction=true)
#
#Computes the velocity at the wake vertices.
#[MODIFIED] Added `interaction` flag to toggle wake interference between different surfaces.
#"""
#function get_wake_velocities!(wake_velocities, surfaces, wakes, ref, fs, Γ,
#    additional_velocity, Vte, symmetric, repeated_points, nwake,
#    surface_id, wake_finite_core, wake_shedding_locations, trailing_vortices, xhat,
...
#    nsurf = length(surfaces)
#
#    # loop through all surfaces (receiving wake points)
#    for isurf = 1:nsurf
#
#        # number of chordwise and spanwise panels
#        nc, ns = size(surfaces[isurf])
...
#
#        # ----------------------------------------------------------------------
#        # 1. Velocity at the wake shedding locations (Trailing Edge)
#        # ----------------------------------------------------------------------
#        for is = 1:ns+1
#
#            # check if this point is a duplicate, skip if it is
...
#
#        # ----------------------------------------------------------------------
#        # 2. Velocity at all other wake vertices
#        # ----------------------------------------------------------------------
#        cr = CartesianIndices((2:nw+1, 1:ns+1))
#
#        # loop through all vertices
...
#
#                # ==============================================================
#                # [MODIFIED] INTERACTION TOGGLE LOGIC
#                # ==============================================================
#                # If interaction is FALSE, and Surface IDs are different,
#                # skip induced velocity calculation.
#                if !interaction && (surface_id[isurf] != surface_id[jsurf])
#                    jΓ += Ns # IMPORTANT: We must still advance the Γ index!
#                    continue
#                end
#                # ==============================================================
#
