...
const SWEEP_CASE_RUNNER = joinpath(
    SWEEP_EXAMPLE_DIR,
    "diagnostics",
...
function child_environment(speed, output_directory, output_label, end_time, hard_angle)
    ...
    environment["CHANG_TEST_SPEED_MPS"] = string(speed)
    environment["CHANG_TEST_END_TIME_S"] = string(end_time)
    environment["CHANG_TEST_WING_SPAN_PANELS"] = get(
        ENV, "CHANG_SWEEP_WING_SPAN_PANELS", "30",
    )
    environment["CHANG_TEST_WING_CHORD_PANELS"] = get(
        ENV, "CHANG_SWEEP_WING_CHORD_PANELS", "5",
    )
    environment["CHANG_TEST_PROP_RADIAL_PANELS"] = get(
