...
function source_fingerprint()
    ...
    append!(paths, [joinpath(@__DIR__, name) for name in
        ("run_chang_core_convergence.jl", "chang_aeroelastic_convergence.jl")])
    example = normpath(joinpath(@__DIR__, "..", "..", ".."))
    push!(paths, joinpath(example, "chang_case.jl"))
    append!(paths, filter(p -> endswith(p, ".jl"), readdir(joinpath(example, "src"); join = true)))
