# Zero-torque windmilling trim for the isolated rigid Chang propeller.
...
options = ChangWindmillingTrimOptions(
    ...
    radial_panels = env_int("CHANG_TRIM_RADIAL_PANELS", 10), #  config.propeller.radial_panels
...
println("Chang windmilling trim")
@printf(
    "  V=%.3f m/s, alpha=%.3f deg, Chang beta75=%.3f deg, grid=%dx%d, azimuth step=%.3f deg\n",
...
output_directory = normpath(get(
    ...
    joinpath(EXAMPLE_DIR, "output", "chang_windmilling_trim"),
))
mkpath(output_directory)

evaluation_path = joinpath(
    output_directory,
    "$(model_label)_windmilling_rpm_evaluations.csv",
)
evaluation_rows = [
    permutedims([
        result.rpm,
...
open(evaluation_path, "w") do io
    println(
        io,
        "rpm,mean_torque_nm,mean_thrust_n,CQ,CT,J,mu," *
...
history_path = joinpath(output_directory, "$(model_label)_windmilling_trim_history.csv")
azimuth_degrees = (
...
summary_path = joinpath(output_directory, "$(model_label)_windmilling_trim_summary.txt")
open(summary_path, "w") do io
    println(io, "Chang isolated-propeller windmilling trim")
    ...
    @printf(io, "Torque at initial RPM: %+.10f N m\n", trim_solution.initial.mean_torque_nm)
    @printf(io, "Windmilling RPM: %.8f\n", trim.rpm)
