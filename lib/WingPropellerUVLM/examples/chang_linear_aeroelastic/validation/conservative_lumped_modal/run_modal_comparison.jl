...
open(report_path, "w") do io
    ...
    println(io, "The production driver now uses the same exact work-conjugate interpolation. The retained 20-element end-to-end smoke artifact predates that integration and attached at y = 6.375 m, so it is retained only as historical solver evidence.")
    println(io)
    ...
    if !isempty(validation_values)
        ...
        println(io)
        println(io, "The retained short trim-plus-impulse result exercised the production UVLM/generalized-alpha path with the conservative arrays. It is a solver smoke test, not a damping-convergence result.")
        println(io)
        validation_fields = (
