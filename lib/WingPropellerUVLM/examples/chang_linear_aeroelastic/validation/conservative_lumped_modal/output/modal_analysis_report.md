...
The production driver now uses the same exact work-conjugate interpolation. The retained 20-element end-to-end smoke artifact predates that integration and attached at y = 6.375 m, so it is retained only as historical solver evidence.
...
The retained short trim-plus-impulse result exercised the production UVLM/generalized-alpha path with the conservative arrays. It is a solver smoke test, not a damping-convergence result.
