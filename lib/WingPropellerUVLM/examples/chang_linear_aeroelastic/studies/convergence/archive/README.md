# Archived convergence workflow

These drivers and presets are retained for reference. Use the two independent
entry files described in [the current README](../README.md) for new studies.
The instructions below describe the older workflow and its former paths.

# Chang two-stage UVLM convergence workflow

For fixed core radii in metres and settings taken from the current
`chang_case.jl`, use the [fixed-radius study](CORE_CONVERGENCE.md): edit
`core_convergence_case.jl` and run `run_chang_core_convergence.jl`.
It provides rigid-aerodynamic and aeroelastic modes. The workflow below
describes the separate factor-based studies and their aerodynamic gate.
...
For the current core dimensions, literature references, and a strategy for
separating core sensitivity from flutter convergence, see
[finite-core selection](FINITE_CORE_SELECTION.md).
...
explicitly sets `CHANG_CORE_RADIUS_M=nothing` so a fixed radius in the editable
main case cannot override the selected factor-based law. It
