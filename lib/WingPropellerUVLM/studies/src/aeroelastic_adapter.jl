function load_model()
    if !isdefined(@__MODULE__,:Model)
        @eval module Model
            include(joinpath(@__DIR__,"..","..","examples","chang_linear_aeroelastic","src","ChangAeroelastic.jl"))
        end
    end
    return nothing
end
function model_defaults(p,mode,c,s,directory)
    ...
    return (;
        ...
        aerodynamic=(core_radius_m=f.radius,segment_core_factor=f.segment,chord_core_factor=f.chord,
            elastic_axis_fraction=p.elastic_axis_fraction),
...
function aeroelastic_case(c,s,selection,directory)
    ...
    completed=run.solution.last_step==length(run.model.parameters.dt)
    solverConverged=completed && all(run.solution.coupling_converged)
    save_toml(joinpath(directory,"analysis_status.toml"),(;completed,solverConverged,
        acceptedSteps=run.solution.last_step,expectedSteps=length(run.model.parameters.dt)))
    completed && solverConverged || error("Aeroelastic run did not converge over the complete discrete time grid")
