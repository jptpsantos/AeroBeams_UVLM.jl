...
Each case is run in a fresh Julia process through the diagnostic wrapper, so
all production structural, aerodynamic, and coupling routines are preserved.
The post-impulse pitch and yaw responses are evaluated with the moving-block
FFT method used by `moving_block.jl`. For every overlapping signal block, the
maximum single-sided FFT magnitude is retained and fitted as
...
Run from the repository root with
    ...
    julia --project=lib/WingPropellerUVLM \
        lib/WingPropellerUVLM/examples/chang_linear_aeroelastic/run_chang_uvlm_convergence_sweep.jl
...
#   2. converts that case into CHANG_TEST_* environment variables;
#   3. launches diagnostics/run_chang_diagnostic_case.jl in a fresh process;
#   4. that wrapper executes an in-memory, parameterized copy of
#      run_chang_linear_aeroelastic.jl;
#   5. reads the resulting pitch/yaw history CSV;
#   6. calculates moving-block damping; and
#   7. compares every refinement level with the last level of its family.
#
...
# ==============================================================================
# 1. EDIT-FRIENDLY STUDY DEFINITION
# ==============================================================================

# The nominal settings match the current primary Chang input, except that a
# common one-revolution retained age is used for both the wing and propeller
# wakes. Wake row counts are generated from wake age and azimuth step.
...
const NOMINAL_FCORE_SEGMENT_FACTOR = 0.5
...
const WING_MESH_LEVELS = [
    (label = "coarse_20x3", span = 20, chord = 3),
    (label = "nominal_30x5", span = 30, chord = 5),
    (label = "fine_40x7", span = 40, chord = 7),
]

# Propeller mesh is radial x chord for each blade.
const PROPELLER_MESH_LEVELS = [
    (label = "coarse_6x6", radial = 6, chord = 6),
    (label = "nominal_10x10", radial = 10, chord = 10),
    (label = "fine_12x12", radial = 12, chord = 12),
]
...
const WAKE_LENGTH_LEVELS_REVOLUTIONS = [0.5, 1.0, 2.0, 3.0]

# The production core law is epsilon = factor * local segment width. Smaller
# factors are ordered later and treated as the refinement direction here.
const FINITE_CORE_LEVELS = [1.0, 0.5, 0.25]
...
const AZIMUTH_STEP_LEVELS_DEG = [10.0, 5.0, 2.5]

# Moving-block settings reproduce moving_block.jl. The FFT block must be a
...
# ==============================================================================
# 2. RUNTIME CONTROLS AND CASE GENERATION
# ==============================================================================

# Directory containing this sweep, the WingPropellerUVLM project activated by
# each child, and the diagnostic wrapper that executes the production driver.
const CONVERGENCE_EXAMPLE_DIR = @__DIR__
const CONVERGENCE_PACKAGE_DIR = normpath(joinpath(CONVERGENCE_EXAMPLE_DIR, "..", ".."))
const CONVERGENCE_CASE_RUNNER = joinpath(
    CONVERGENCE_EXAMPLE_DIR,
    "diagnostics",
    "run_chang_diagnostic_case.jl",
)
...
- `fcore_segment_factor::Float64`: finite-core radius divided by segment width
...
function positive_peak_indices(signal)
    # This is the local-maximum operation used by Peaks.argmaxima in the
    # original moving_block.jl, written locally to keep the sweep independent
    # of Peaks.jl. Positive maxima define the beginning and end of the segment.
    peak_indices = Int[]
...
Applies the moving-block FFT damping method from `moving_block.jl` to one response channel
...
function moving_block_metrics(
    ...
    # Step 2: reproduce moving_block.jl's peak cutoff. Peaks below one percent
    # of the maximum absolute response are ignored as numerical noise.
    ...
    # Step 4: adapt the power-of-two FFT block to the analyzed record length.
    # Refining dt creates more samples, so this adjustment tends to preserve a
    # comparable physical block duration between time-step cases.
    ...
    # Step 5: slide the block by one sample. For each block, form the
    # single-sided spectrum and retain its largest magnitude. Taking the
    # maximum follows moving_block.jl and tracks the dominant modal component.
    block_count = sample_size - block_size + 1
    log_block_amplitude = Vector{Float64}(undef, block_count)
    for block_start in 1:block_count
        block = @view analyzed_signal[block_start:block_start+block_size-1]
        ...
        maximum_amplitude = maximum(single_sided)
        maximum_amplitude > 0 || return unavailable_metrics(
            length(peak_indices);
            block_size,
            block_count,
        )
        log_block_amplitude[block_start] = log(maximum_amplitude)
    end
    ...
    block_time = analyzed_time[1:block_count]
    ...
    # Step 7: positive-peak spacing gives the damped frequency. The moving
    # block slope is then converted to a conventional damping ratio. Lambda is
    # still the primary convergence quantity because it is the direct output
    # of the requested moving-block method.
    retained_peak_indices = peak_indices[peak_from_start:end-peak_from_end]
    retained_peak_times = fit_time[retained_peak_indices]
    mean_period = sum(diff(retained_peak_times)) / (length(retained_peak_times) - 1)
...
Creates the environment dictionary consumed by `chang_diagnostic_case.jl` and the production Chang driver
...
function child_environment(
    ...
    # The diagnostic wrapper replaces chang_case.jl with
    # diagnostics/chang_diagnostic_case.jl. That diagnostic case reads the
    # CHANG_TEST_* variables assembled here. Consequently, the convergence
    # cases do not use the mesh/speed defaults in the production chang_case.jl.
    # copy(ENV) preserves Julia/package settings from the parent process.
    environment = copy(ENV)
    rows = wake_rows(case)

    # Physical operating point and duration.
    environment["CHANG_TEST_SPEED_MPS"] = string(speed_mps)
    environment["CHANG_TEST_AOA_DEG"] = get(ENV, "CHANG_CONVERGENCE_AOA_DEG", "0.0")
    environment["CHANG_TEST_TRIM_RPM"] = string(trim_rpm)
    environment["CHANG_TEST_TRIM_SPEED_MPS"] = string(trim_speed_mps)
    environment["CHANG_TEST_END_TIME_S"] = string(end_time_s)
    # CHANG_END_TIME_S has precedence inside chang_model_parameters.jl.
    environment["CHANG_END_TIME_S"] = string(end_time_s)

    # Aerodynamic grids and azimuth-based time step.
    environment["CHANG_TEST_WING_SPAN_PANELS"] = string(case.wing_span_panels)
    environment["CHANG_TEST_WING_CHORD_PANELS"] = string(case.wing_chord_panels)
    environment["CHANG_TEST_PROP_RADIAL_PANELS"] = string(case.prop_radial_panels)
    environment["CHANG_TEST_PROP_CHORD_PANELS"] = string(case.prop_chord_panels)
    environment["CHANG_TEST_AZIMUTH_STEP_DEG"] = string(case.azimuth_step_deg)
    ...
    environment["CHANG_TEST_WAKE_ROWS_WING"] = string(rows)
    environment["CHANG_TEST_WAKE_ROWS_PROP"] = string(rows)
    environment["CHANG_TEST_FCORE_SEGMENT_FACTOR"] = string(case.fcore_segment_factor)
    environment["CHANG_TEST_FCORE_CHORD_FACTOR"] = "0.0"

    # Aerodynamic-model switches. The same force-model value is sent to both
    # the diagnostic case and diagnostic coupling adapter.
    environment["CHANG_TEST_INTERACTION"] = get(
        ENV,
        "CHANG_CONVERGENCE_INTERACTION",
        "false",
    )
    environment["CHANG_TEST_BASE_FORCE_MODEL"] = get(
        ENV,
        "CHANG_CONVERGENCE_FORCE_MODEL",
        "imperial",
    )
    environment["CHANG_TEST_FORCE_MODEL"] = get(
        ENV,
        "CHANG_CONVERGENCE_FORCE_MODEL",
        "imperial",
    )
    environment["CHANG_TEST_PROP_MOMENT_PROJECTION"] = get(
        ENV,
...
function run_case(
    ...
    # Execute the diagnostic wrapper, not run_chang_linear_aeroelastic.jl
    # directly. The wrapper injects this case's environment-controlled mesh,
    # wake, core, and time-step values into the production driver in memory.
...
# ==============================================================================
# 5. CONVERGENCE COMPARISON AND OUTPUT
# ==============================================================================
...
function write_text_report(
    ...
    open(path, "w") do stream
        ...
        println(stream, "------")
        println(stream, "The signal is cut between selected positive peaks, following moving_block.jl.")
        ...
        println(stream, "- The finite core is proportional to local segment width. That family is a")
...
function main()
    ...
    # Radius is fixed by chang_diagnostic_case.jl; it cancels from the
