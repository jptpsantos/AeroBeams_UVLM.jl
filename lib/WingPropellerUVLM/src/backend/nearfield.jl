...
"""
    imperial_nodal_positions(surface)

Return the `(nc+1, ns+1)` vortex-vertex positions associated with the nodal
forces produced by [`imperial_nodal_forces`](@ref). Load-transfer moments must
use these positions, rather than the original physical grid, to keep forces
and moment arms on the same aerodynamic lattice.
"""
function imperial_nodal_positions(surface::AbstractMatrix{<:SurfacePanel})
    nc, ns = size(surface)
    nc > 0 && ns > 0 || throw(ArgumentError("surface must contain at least one panel"))

    position_type = typeof(top_left(surface[1, 1]))
    positions = Matrix{position_type}(undef, nc + 1, ns + 1)

    for i in 1:nc, j in 1:ns
        positions[i, j] = top_left(surface[i, j])
    end
    for i in 1:nc
        positions[i, ns + 1] = top_right(surface[i, ns])
    end
    for j in 1:ns
        positions[nc + 1, j] = bottom_left(surface[nc, j])
    end
    positions[nc + 1, ns + 1] = bottom_right(surface[nc, ns])
    return positions
end

"""
    imperial_nodal_positions(system)

Return the vortex-vertex position matrix for every aerodynamic surface in
`system`, in the same order and shapes as `imperial_nodal_forces(system)`.
"""
imperial_nodal_positions(system::System) =
    [imperial_nodal_positions(surface) for surface in system.surfaces]
